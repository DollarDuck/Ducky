...
class Chart2 extends Component {
  ...
  render() {
    ...
    const outputMessage = messageToDisplay(
            ...
            datasets: [
              {
                ...
                label: 'Cumulative Net Present Value (NPV) Without Grad School',
                data: chartData.currentSalaryNPVCum,
                fill: false
              },
              {
                backgroundColor: 'blue',
                label: 'Cumulative Net Present Value (NPV) With Grad School',
        ...
        <Container fluid centered>
          <Statistic size="small" className="padding-left">
            <Statistic.Value>
              {' '}
              {commaFormat(
                chartData.currentSalaryNPVCum[
                  chartData.currentSalaryNPVCum.length - 1
                ]
              )}{' '}
            </Statistic.Value>
          ...
          <Statistic size="small" className="padding-left">
            <Statistic.Value>
              {' '}
              {commaFormat(
                chartData.expectedSalaryNPVCum[
                  chartData.expectedSalaryNPVCum.length - 1
                ]
              )}{' '}
            </Statistic.Value>
...
function messageToDisplay(years, age) {
  ...
  if (years < 5) {
    message =
      'Based on our math, it looks like going to grad school is a really good investment.'
  } else if (years < 10) {
    message =
      'It will take time to recoup your investment, but it looks grad school is a viable path for you'
  } else if (years < 20) {
    message =
      'It will take over a decade to recoup your investment but it could still be worth it. Think hard about your decision'
  } else {
    message =
      'Based on our calculations, it will take ' +
      years +
      ' for your decision to make sense financially. You will be ' +
      (years + age) +
      '!'
  }
  return message
